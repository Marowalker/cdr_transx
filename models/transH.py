...
import os
import numpy as np
from data_utils import load_vocab, Timer, Log
...
class TransHModel:
    def __init__(self, model_path, batch_size, epochs, score):
        ...
        self.ent_size = len(load_vocab(constants.ALL_WORDS))
        self.rel_size = len(load_vocab(constants.ALL_DEPENDS))
        self.epochs = epochs
    ...
    def transfer_to_hyperplane(e, normal):
        """Transfer the embeddings to the hyperplane defined by the normal"""
        ...
        ones_for_sum = np.ones((e.shape[1], e.shape[1]))
    ...
    def _add_embeddings(self):
        ...
        self.head_lookup = self.entity_embeddings(self.head)
        ...
        self.tail_lookup = self.entity_embeddings(self.tail)
        ...
        self.head_neg_lookup = self.entity_embeddings(self.head_neg)
        ...
        self.tail_neg_lookup = self.entity_embeddings(self.tail_neg)
    ...
    def load(self, load_file=None, embed_type=None):
        if not os.path.exists(load_file):
            ...
            for weight in all_weights:
                if embed_type in weight.name:
                    all_embeddings = weight.numpy()
                    f = open(load_file, 'wb')
